...
// FIXED:
//   1. `const { id } = useParams()` → `const { ticker } = useParams()`
//      Route is /company/:ticker so 'id' was always undefined → every fetch was
//      hitting GET /companies/undefined → 404 → blank page every time.
//   2. Endpoint changed to GET /companies/ticker/:ticker (returns full data
//      including metrics + historicalPrices).
//   3. Added Authorization header on all fetches.
//   4. Full profile layout: price header, key stats, metrics grid,
//      price chart, recent reports, add/remove watchlist button.
...
export default function CompanyProfilePage() {
  // ✅ FIX 1: correct param name matches the route /company/:ticker
  ...
  const fetchCompany = useCallback(async () => {
    ...
    setError(null);
    try {
      // ✅ FIX 2: correct endpoint — /companies/ticker/:ticker
      // ✅ FIX 3: added Authorization header
